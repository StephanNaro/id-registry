...
Rust Server (Rocket) ───► SQLite DB (id\_registry.db)
...
\- IDs are stored in `ids` table with owner, table\_name, confirmed, deleted, created\_at
...
cmake -DBUILD\_DEVELOPMENT\_MODE=OFF -G "Ninja" ..\\..
...
\- Run the GUI once → set database path (e.g. `C:\\Users\\YourName\\AppData\\Local\\id\_registry.db`) → Save \& Initialize
...
curl -X POST http://127.0.0.1:8000/generate -H "Content-Type: application/json" -d "{\\"owner\\":\\"person\_app\\",\\"table\\":\\"contacts\\"}"
...
curl -X POST http://127.0.0.1:8000/confirm -H "Content-Type: application/json" -d "{\\"id\\":\\"sbdhRrxf5GIB\\"}"
...
curl http://127.0.0.1:8000/get\_id/sbdhRrxf5GIB
...
INSERT OR IGNORE INTO settings (key, value) VALUES ('id\_length', '12');
...
&nbsp;   $data = json\_decode($response->getBody(), true);
